...
<!-- TASK_FLOW_RULES_BEGIN -->
# タスク運用ルール（現場）｜意味監査反映（確定）

## 状態（status）— 6本固定
1. 受注カード
2. 見積もり→連絡待ち
3. 調達/発注関連（必要なら）
4. 納品待ち→日時調整待ち（= 顧客連絡フェーズ）
5. 日時確定→作業待ち
6. 入金待ち（当面は手動で完了）

※日時入力は「仮登録」であり、状態遷移トリガーにしない。

## 状態遷移の原則
- 状態遷移は **手動ありき**（自動化は補助・提案・ガードのみ）
- 4は「顧客連絡」を確定させる状態だが、遅れ等により 1/3 でも連絡が発生し得る（日時は状態トリガーにしない）

## 3 → 4 の扱い（警告ルール）
- 3のまま4へ進めるのは許可（現場運用上必要）
- ただし、3→4へ移行した場合は **必ずコメントで警告**を残す：
  - 「納品未確認のまま4へ移行」
  - 「残タスク：納品確認」

## 書類（docType）と発行方針
- 書類は常時PDF生成しない。**生成指示（UI/コメント）でPDF生成**する（オンデマンド）
- 0円のとき：見積/請求/領収は原則発行しない
- 領収書/請求書は 0円では発行しない

## タスク完了の最低条件（通常）
- コピペ媒体で送れる完了報告の情報が揃っていること
- 領収書を作成できる情報が揃っていること（= 住所は必須、電話は必須ではない）
- 金額が 0円 または未記入で完了は不可（下の例外を除く）

## 例外完了（作業メニューで管理）
以下の作業が選択/記録されている場合のみ、0円/未記入でも完了可能：
- WM_900（見積のみ）
- WM_901（キャンセル）
- WM_902（無料対応）

上記以外で 0円/未記入のまま完了を押した場合：
- コメントで警告を出す
- 完了を取り消し（復旧）

## 連絡経路の記録（任意）
- 連絡経路（電話/SMS/LINE/メール/その他）を任意で記録する（後でUIに反映）
<!-- TASK_FLOW_RULES_END -->


<!-- STATE_SUGGEST_RULES_BEGIN -->
# 状態遷移の自動判定（提案）｜フェーズ②（確定）

## 前提
- 状態の確定は **手動が正**（自動は提案・ガードのみ）
- 日時入力は「仮登録」であり、状態遷移のトリガーにしない
- 3→4 の手動移行は許可。ただし警告コメント（納品未確認/残タスク）を必ず残す

## 状態提案ルール（S01〜S06）
- S01（受注カード）:
  - 受注/成約/交渉開始のコピペが入ったら提案
- S02（見積もり→連絡待ち）:
  - 現地見積/交渉中/希望日調整が必要な場合に提案
- S03（調達/発注関連）:
  - 使用部品の確定や調達が発生した場合に提案
  - ※S03はスキップ可能（当日買い出し/工賃のみ等）
- S04（納品待ち→日時調整待ち）:
  - 顧客連絡を開始する段階で提案
  - ※S01/S03でも連絡は発生し得る（遅れ等）
- S05（日確→作業待ち）:
  - 日時が確定し「作業待ち」にしたいときに提案（自動では遷移しない）
- S06（入金待ち）:
  - 当面は手動で完了（自動化は保留）

## 完了報告の最終判定（完了ボタンのガード）
完了確認コメントは省略形で生成し、以下を満たさない場合は ❌ として完了を復旧する。

### 最優先：確定申告チェック（不足があれば必ず❌）
- 顧客名
- 住所（番地・建物名まで）
- 作業日（訪問日時）
- 作業内容（work_menu 複数可）
...
- 2回目：◯
- 3回目以降：◎
- 既存顧客（◯/◎）の場合、コンシェルジュがコメントに「過去の作業＋日時」を追記する

<!-- STATE_SUGGEST_RULES_END -->


<!-- PASTE_INPUT_CONTRACT_BEGIN -->
# コピペ入力仕様（受注・完了報告）｜確定

## 目的
- 現場は「全文コピペ」だけで進む
- システム側が自動抽出し、最後は 0（承認）で確定
...
### KURAMA（くらま）
- 定型文から抽出できる範囲を採用する
- 作業内容が文面に無い場合は空欄（後で work_menu から手動追加）

### MITSUMOA（ミツモア）
- 「回答内容以下」は信頼できないため抽出対象から除外
- 作業内容は原則空欄（未確定）
- ID/金額/決済方法/日程/住所/電話など上段の確定情報のみ採用

## 完了報告コピペ：最終判定（完了ボタンのガード）
### 最優先：確定申告チェック（不足があれば必ず❌）
- 顧客名
- 住所（番地・建物名まで）
- 作業日（訪問日時）
...
<!-- PASTE_INPUT_CONTRACT_END -->


<!-- WORK_AUTOFILL_AND_CONFIRM_FMT_BEGIN -->
# 作業コード（複数）自動付与 ＆ 完了確認コメント（最終）｜確定

## 作業コードは複数可（必須）
- タスクは work_menu を **複数**保持できる（配列）
- 自動付与＋手動追加の両方を許可する

## 使用部品→作業コードの自動付与（補助）
- 使用部品に「蛇口」系が含まれる場合：水栓交換系（混合/単水栓など）を候補追加
- 使用部品に「トイレ」系が含まれる場合：トイレ系（便器/タンク/便座/ウォシュレット等）を候補追加
- 複数該当する場合：該当する作業コードをすべて候補追加
- 最終確定は手動が正（自動は提案・補助）

## ミツモア（MITSUMOA）の補完方針
- 回答内容以下は捨てる（抽出対象外）
- 作業内容は空欄になり得る
- ただし、タスクの使用部品や完了報告から自動候補を付けて良い（上記の部品→作業）

## 完了確認コメント（省略形・一時UI）
コメントは履歴用途ではなく「このまま完了してよいか」の最終確認用とする。
...
<!-- WORK_AUTOFILL_AND_CONFIRM_FMT_END -->


<!-- REPEAT_HISTORY_COMMENT_BEGIN -->
# 既存顧客（◯/◎）の履歴コメント｜確定

## 目的
- 既存顧客（◯/◎）の場合、現場が即座に履歴を把握できるようにする
- コメントは「履歴の入口」として最小情報を自動挿入する

## 仕様
- 既存顧客フラグが ◯ または ◎ の場合：
  - コンシェルジュがコメント欄に「過去の作業＋日時」を追記する（必須）
- 記載形式は省略形で良い（例）：
  - 「履歴：2024/11/19 詰まり除去」
  - 「履歴：2025/08/03 水栓交換」

## 自動化の方針
- 将来的に顧客検索（住所/電話/名前）で履歴を引けるようにする
- 現段階は手動追記でも運用可（仕様としては必須項目扱い）
<!-- REPEAT_HISTORY_COMMENT_END -->
