...
【生成物完全性チェック（Completeness Guard）】

本 system_protocol は、生成フェーズ（C）で生成された成果物に対し、
意味・仕様・業務内容を一切解釈せず、
「生成物として成立しているかどうか」のみを機械的に検査する。

本チェックは、T（監査）および R（差戻し）に入る前に実行される
前段安全弁として位置づけられる。

以下のいずれかに該当する生成物は、
「生成失敗（Incomplete Generation）」として即時 NG 判定とする。

・生成物に明示的な完了宣言が存在しない
  （例：END OF CODE / END OF FULL CODE 等）

・分割出力が指定されているにもかかわらず、
  連番（PART n / N）が欠落している、または最終 PART が存在しない

・生成物中に以下の省略・未完了を示す表現が含まれる
  （例：「省略」「以下略」「…」「TODO」「ここから先」等）

・ブロック構造（関数・クラス・条件分岐・ループ等）が
  構文上閉じられていない状態で終了している

本チェックは、
旧成果物・差分・仕様内容との比較を行わず、
生成物単体の形式的完全性のみを対象とする。

本チェックで NG と判定された場合：

・当該生成物は正式成果物として扱わない
・master_spec、業務状態、確定データには一切影響を与えない
・状態は R（差戻し）フェーズへ遷移する
・再生成要求を発行する

本チェックは、
環境依存・実装依存・言語仕様の正誤を判断しない。
生成物が「完成品として成立しているかどうか」のみを保証する。
