...
$repoLabel = $(if ($slug) { $slug } else { $remote })
Write-Host (Out "repo"   $repoLabel)
...
try {
  if ($slug) {
    ...
    $json = gh pr list --state merged --limit 1 --json number,mergedAt,mergeCommit,headRefName,title 2>$null | ConvertFrom-Json
  }
