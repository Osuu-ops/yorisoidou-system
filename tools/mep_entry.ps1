  ...
  try {
    $env:MEP_AUTO_LOOP_EXIT = [string]$exit
    if (-not $env:MEP_SRC) { $env:MEP_SRC = "DRAFT" }
    if (-not $env:MEP_PRE) { $env:MEP_PRE = "OK" }
    $env:MEP_PROGRESS_EXIT = [string]$exit
    if ($gateMax -ge 0) {
      if ($exit -eq 0) { $env:MEP_PROGRESS_LABEL = ("G{0}/{0} ALL_DONE" -f $gateMax) }
    ...
    if (Test-Path -LiteralPath $reporter) { & $reporter -ExitCode $exit }
  } catch {}
