...
- 完全OS完成（A+B 完了）：SSOT_SCAN / CONFLICT_SCAN / EXTRACT / Self-heal完全 を統合し、post-writeback + loop owned phase resume、canonical engine run completion 追跡、child run_state 解釈、主要 manual reason_code の canonical self-heal mapping 拡張と `WAIT_LOOP_ENGINE` の unresolved / child-state-unavailable fallback の deterministic recovery まで含めて仕様本体までrunnerが責任を持つ
