...
#   (3) portfolio_id keys (latest exists)  [quick overview]
#   (4) Latest BOOT per portfolio_id (copy/paste into NEW CHAT top)
#
# Notes:
# - $PID is a reserved automatic variable in PowerShell -> NEVER use it.
...
$IncludeLegacyPlaceholder = $false   # $true => include "...Z"/"CHAT_..." rows too
# =============================================================================
# Sync (read-only)
# =============================================================================
...
function Extract-JsonObjectAt([string]$s, [int]$pos) {
  ...
  # Walk right with brace counting (string-aware)
  $depth = 0
  $inStr = $false
  $esc = $false
...
function Try-ParseDateUtc([string]$s) {
  ...
  # DateTimeOffset parse (handles Z / offsets well)
...
# =============================================================================
# Extract CHECKPOINT_OUT objects
# =============================================================================
...
foreach ($m in $matches) {
  $json = Extract-JsonObjectAt $raw $m.Index
  ...
  if ($null -ne $o) {
    ...
    $anchor   = Get-Prop $o "primary_anchor"   # may be absent
    $mainHead = Get-Prop $o "main_head"
    $fixedVer = Get-Prop $o "fixed_handoff_version"
    $mode     = Get-Prop $o "mode"
  } else {
    # regex fallback
    ...
    $mainHead = Try-Get $json "main_head"
    $fixedVer = Try-Get $json "fixed_handoff_version"
    $mode     = Try-Get $json "mode"
  }
  if ([string]::IsNullOrWhiteSpace($pfid)) { $pfid = "UNSPECIFIED" }
  if (-not $IncludeLegacyPlaceholder) {
...
$sorted = @($items | Sort-Object checked_at_dt -Descending)
# Latest per portfolio_id
$latest = @{}
foreach ($x in $sorted) {
...
""
"=============================="
"MEP BOOT LIST EXPORT (FINAL / READABLE)"
("GENERATED_AT_LOCAL=" + (Get-Date).ToString("yyyy-MM-dd HH:mm:ss K"))
("HEAD(main)=" + $headSha)
("LEDGER_PATH=" + $ledgerPath)
("CHECKPOINT_OUT_MATCHES=" + $matches.Count)
("CHECKPOINT_OUT_EXTRACTED=" + $sorted.Count)
...
foreach ($pfidKey in ($latest.Keys | Sort-Object)) {
  $x = $latest[$pfidKey]
  ""
  ("##### portfolio_id={0} | checked_at_utc={1} | phase={2}" -f $pfidKey, $x.checked_at_utc, $x.current_phase)
