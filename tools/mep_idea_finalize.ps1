...
# Usage:
#   1) Ensure implemented receipts exist in docs/MEP/IDEA_RECEIPTS.md (RESULT: implemented)
#   2) .\tools\mep_idea_finalize.ps1 1 3
# Effect:
#   - Verifies receipts for selected ideas
#   - Deletes them from IDEA_VAULT ACTIVE
#   - Refreshes IDEA_INDEX
...
# Verify receipts: each IDEA_ID must appear in a line that also contains "RESULT: implemented"
...
# Delete selected blocks from ACTIVE
...
# Remove blocks whose IDEA_ID is in $sel
...
Write-Host ("OK: Deleted {0} implemented idea(s) from ACTIVE (verified by receipts)." -f $removed)
