...
def main():
    ...
    # INPUT_PACKET (8-gate compatible, but does NOT call it)
    packet = []
    packet.append("# INPUT_PACKET (for 8-gate entry)")
    packet.append(f"ISSUE: #{num}")
    packet.append(f"LANE: {lane}")
    packet.append(f"TIMESTAMP_UTC: {utc_z()}")
    packet.append("MODE: STANDALONE_PRE_8GATE")
    packet.append("SAFE: does_not_call_8gate")
    packet.append("")
