...
### 7.1 類似表現（表記ゆれ）もトリガとして扱う（固定）

目的：
- 表記ゆれ・誤字・言い回し違いでも、同じ一本道を開始できるようにする。

AIの判定ルール（固定）：
- 次のいずれかに該当したら、該当トリガとして扱う（完全一致を要求しない）。

1) アイデアまとめて 系（統合タイミング提案＋最終監査を必ず出す）
   - 例：アイデアまとめる／アイデア整理／アイデア統合順／アイデア統合タイミング／案をまとめて

2) アイデア収集 系（capture用1ブロックを出す）
   - 例：アイデア吸い上げ／アイデア保存／アイデア追加／アイデア記録／アイデアを入れる／アイデアを避難

3) 最終監査 系（Go/No-Goを必ず出す）
   - 例：監査して／矛盾チェック／危険チェック／やめたほうがいい？／GO NO-GO／最後に確認

注意：
- 曖昧な場合でも、AIは質問で止めずに、想定したトリガを明示して Section 5/6 を先に提示する。
