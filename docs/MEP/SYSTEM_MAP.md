...
D) Self-heal完全版：push retry / gh fallback / check再取得 / reason_code→自動復帰（主要 manual reason_code mapping、structural/manual hard stop の一部 status 化、governance / PR lifecycle reason_code の一部 canonical command 化、persistent loop structural reason の hard stop 維持理由と canonical inspection=status 明文化、environment / patch prerequisite hard stop の canonical observation=status 整理、loop_state / handoff の durable-first cleanup と evidence / PR ambiguity 側の remembered pr_url fallback + zero-open-PR handoff to `pr-create` までは拡張済、全辞書化は未）
