...
# NOTE: do not rely on $args (may lose bound params). Parse $MyInvocation.Line first.
### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
...
try {
  ...
  if ($line -match '(i)\-PrNumber\s+(\d+)') {
    [int]::TryParse($Matches[1], [ref]### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done."_DoneB_PrNumber -le 0) {
  for ($i = 0; $i -lt $args.Count; $i++) {
    if ([string]$args[$i] -ieq "-PrNumber" -and ($i + 1) -lt $args.Count) {
      [int]::TryParse([string]$args[$i+1], [ref]### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done."_DoneB_PrNumber -gt 0) {
  try {
    $filesJson = (gh pr view ### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done."_DoneB_PrNumber --repo $repo --json files 2>$null)
    if (-not $filesJson) { throw "gh pr view failed for PR #### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done."_DoneB_PrNumber" }
    ...
    $files = @()
    if ($obj -and $obj.files) {
      $files = @($obj.files | ForEach-Object { ### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done.".path } | Where-Object { ### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done." -and ### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
...
Info "Unified entry step1 done.".Trim() } | Sort-Object -Unique)
    ...
    Write-Host "## Scope-IN candidates"
    Write-Host ("- [TOOLING_ERROR] " + ### DONEB_PRNUMBER_SHIM_V1 ###
# DoneB②③: -PrNumber non-interactive route (Scope-IN candidates; bullet-only)
# NOTE: do not touch existing param(); parse $args to avoid breaking legacy entry.
$__DoneB_PrNumber = 0
for ($i = 0; $i -lt $args.Count; $i++) {
