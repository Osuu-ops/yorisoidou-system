...
- evidence / PR ambiguity 側は `EVIDENCE_REQUIRED_BUT_UNAVAILABLE` の remembered `pr_url` fallback と、`PR_NOT_FOUND_FOR_RUN` / `PR_CLOSED_FOR_RUN` の zero-open-PR からの `pr-create` handoff まで自動継続する
- `MULTIPLE_PR_FOR_ONE_RUN` は truly ambiguous な複数 open PR とみなし、自動解消せず canonical observation 付き manual stop に残す
