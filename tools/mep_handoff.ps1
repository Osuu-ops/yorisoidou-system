...
function Get-MepEvidenceAuditMarkers {
  param(
    [Parameter(Mandatory=$true)][string]$EvidenceBundleAbs,
    [int]$Tail = 400,
    [int]$Take = 5
  )
  if (-not (Test-Path -LiteralPath $EvidenceBundleAbs)) {
    throw "EVIDENCE_BUNDLE not found: $EvidenceBundleAbs"
  }
  $lines = Get-Content -LiteralPath $EvidenceBundleAbs -Encoding UTF8 -Tail $Tail
...
try {
  ...
  $markers = Get-MepEvidenceAuditMarkers -EvidenceBundleAbs $evidenceAbs -Tail 400 -Take 5
  if ($markers -and $markers.Count -gt 0) {
    foreach ($ln in $markers) { $out.Add("* - " + $ln) }
  } else {
    $out.Add("* - EVIDENCE_BUNDLE抽出不能（audit=OK,WB0000 が Tail 400 に存在しない）: " + $evidenceRel)
  }
} catch {
  $out.Add("* - EVIDENCE_BUNDLE抽出不能（例外）: " + (
function Get-BundledAtFromBundled([string]$bundledPath){
  if (-not (Test-Path $bundledPath)) { return $null }
  $m = Select-String -Path $bundledPath -Pattern '^\s*BUNDLED_AT\s*=\s*(.+)\s*$' -AllMatches -ErrorAction SilentlyContinue | Select-Object -First 1
  if ($m) { return $m.Matches[0].Groups[1].Value.Trim() }
...
# --- /fallback excerpt ---
 ...
 -match 'audit=OK,WB0000' } | Select-Object -Last $Take
  return ,$picked
}
function Info([string]$m){ Write-Host $m -ForegroundColor Cyan }
