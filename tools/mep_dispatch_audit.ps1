...
$script:PathRegex = $null
try { $script:PathRegex = $PathRegex } catch { $script:PathRegex = "(?i)(writeback|bump_bundle|sync_evidence|handoff_push)" }
if (-not $script:PathRegex) { $script:PathRegex = "(?i)(writeback|bump_bundle|sync_evidence|handoff_push)" }
$target = $all | Where-Object { $_.path -match $script:PathRegex }
param(
  # 監査対象を "writeback系" に絞るためのパス正規表現（既定: writeback / bump bundle / sync evidence / handoff push）
  [string]$PathRegex = "(?i)(writeback|bump_bundle|sync_evidence|handoff_push)",
  # workflow_dispatch の上限（監査対象に対して）
